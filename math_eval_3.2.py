...
def prepare_data(data_name, args):
    ...
    # get out_file name
    out_file = args.input_file.replace(".json", "_3.2.josn")
...
def main(llm, data_name, args):
    ...
    for i, example in tqdm(enumerate(examples), total=len(examples)):
        if np.mean(example["score"]) == 0 or np.mean(example["score"]) == 1:
            continue

        for i, output in enumerate(example["model_output"]):
        ...
        results.append(
            {
                ...
                "full_trace_score": sample["full_trace_score"],
                "full_trace": sample["full_trace"],
            }
        )
